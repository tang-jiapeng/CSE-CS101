...
Set<string> dfs(const Vector<string>& init, Vector<string>& res,int point){
    Set<string>ans;
    if(point==init.size()){
        string now;
        for(auto i:res) now+=i;
        ans+=now;
        return ans;
    }
    if(check(init[point])){
        string tmp1=toUpperCase(init[point]);
        string tmp2=init[point];
        res[point]=tmp1;
        ans+=dfs(init,res,point+1);
        res[point]=tmp2;
        ans+=dfs(init,res,point+1);
    }else{
        ans+=dfs(init,res,point+1);
...
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Vector<string> init=tokenize(sentence);
    for(int i=0;i<init.size();i++) init[i]=toLowerCase(init[i]);
    return dfs(init,init,0);
}
