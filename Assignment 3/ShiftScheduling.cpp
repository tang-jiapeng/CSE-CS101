...
Set<Shift> dfs(const Set<Shift>& shifts,const Set<Shift>& init,int maxHours){
    if(shifts.size()==0){
        Set<Shift> ans={};
        ans+=init;
        return ans;
    ...
    Set<Shift> remainding=shifts-first,choose=init+first;
    double sum=0;
    bool flag=1;
    for(auto i:choose) sum+=lengthOf(i);
    if(sum>maxHours) flag=0;
    for(auto i:init){
        if(overlapsWith(first,i)) {
            flag=0;
            break;
        }
    }
    if(flag){
        auto x=dfs(remainding,init,maxHours);
        auto y=dfs(remainding,choose,maxHours);
        int sum1=0,sum2=0;
        for(auto i:x) sum1+=valueOf(i);
        for(auto i:y) sum2+=valueOf(i);
        if(sum1>sum2) return x;
        else return y;
    }
    else return dfs(remainding,init,maxHours);
}
