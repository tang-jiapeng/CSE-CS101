...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    string pos1=possibleLinks.firstKey();
    for(auto pos2:possibleLinks[pos1]){
        bool flag=1;
        for(auto i:matching){
            if(i.first()==pos2||i.second()==pos2) {
                flag=0;
                break;
            }
        }
        if(flag){
            Set<Pair> now=matching+(Pair){pos1,pos2};
            auto remainding=possibleLinks;
            remainding.remove(pos1);
            remainding.remove(pos2);
            if(hasPerfectMatching(remainding,now)){
                matching+=now;
                return true;
            }
        }
