...
bool check(double x,double y,const Grid<double>& terrain,double height){
    if(terrain.inBounds(x,y)){
        if(terrain[x][y]<=height) return true;
    }
    return false;
}
...
void BFS(Grid<bool>& ans,const Grid<double>& terrain,
    ...
    while(!que.isEmpty()){
        GridLocation g=que.dequeue();
        for(int i=0;i<4;i++){
            int tx=g.row+dx[i];
            int ty=g.col+dy[i];
            if(check(tx,ty,terrain,height)){
                if(ans[tx][ty]==0){
                    ans[tx][ty]=1;
                    que.enqueue({tx,ty});
                }
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> ans=Grid<bool>(terrain.numRows(),terrain.numCols());
    ans.fill(false);
    BFS(ans,terrain,sources,height);
    return ans;
}
