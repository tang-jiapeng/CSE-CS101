...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength<0)  error("negative");
    else if(kGramLength==0)  error("zero");
    else if(kGramLength>str.length()) return ans;
    else{
        int now=0;
        while(now+kGramLength<=str.length()){
            string temp=str.substr(now,kGramLength);
            ans[temp]++;
            now++;
        }
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
     ...
     * this function.
     */
    if(input.equals({})) error("no input");
    double sum=0;
    for(auto i:input) sum+=(input[i]*input[i]);
    if(sum==0) error("zero");
    Map<string,double> ans={};
    sum=sqrt(sum);
    for(auto i:input) ans[i]=input[i]/sum;
    return ans;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if(numToKeep==0) return ans;
    PriorityQueue<string> que;
    for(auto i:source) que.enqueue(i,source[i]);
    while(!que.isEmpty()&&que.size()>numToKeep) que.dequeue();
    while(!que.isEmpty()){
        string pos=que.dequeue();
        ans[pos]=source[pos];
    }
    return ans;
}
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    double ans=0;
    for(auto i:lhs){
        if(rhs.containsKey(i)) ans+=lhs[i]*rhs[i];
    }
    return ans;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    string name="";
    double ans=0;
    if(corpora.isEmpty()) error("empty");
    else {
        for(auto i:corpora){
            double res=cosineSimilarityOf(textProfile,i.profile);
            if(res>ans){
                ans=res,name=i.name;
            }
        }
