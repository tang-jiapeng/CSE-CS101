...
GridLocation selectdirection(GridLocation cur,int select){
    switch (select){
        case 0 : cur.row++; break;
        case 1 : cur.row--; break;
        case 2 : cur.col++; break;
        case 3 : cur.col--; break;
    }
    return cur;
}
...
Set<GridLocation> generateValidMoves(Grid<bool>& maze, GridLocation cur) {
    ...
    for(int i=0;i<4;i++){
        GridLocation nex=selectdirection(cur,i);
        if(nex.row>=maze.numRows()||nex.col>=maze.numCols()||nex.row<0||nex.col<0||maze[nex]==false){
            continue;
        }else{
            neighbors.add(nex);
        }
...
void validatePath(Grid<bool>& maze, Stack<GridLocation> path) {
    ...
    if(path.size()<maze.numRows()+maze.numCols()-1){
        error("invalid move form Exit to Enter");
    }
    Set<GridLocation> vis;
    GridLocation last=path.pop();
    vis.add(last);
    if(last==mazeEnter&&path.isEmpty()) return;
    while(path.size()>1){
        GridLocation cur=path.peek();
        Set<GridLocation> validMoves=generateValidMoves(maze,last);
        if(!validMoves.contains(cur)) {
            error("invalid move");
        }
        if(vis.contains(cur)) {
            error("it has been visited");
        }
        last=path.pop();
        vis.add(last);
    }
    GridLocation end=path.pop();
    if(end!=mazeEnter||vis.contains(end)) {
        error("the path is error");
    }
}
...
STUDENT_TEST("selectdirection , check is true"){
    GridLocation center = {2, 2};
    int choose=1;
    EXPECT_EQUAL(selectdirection(center,choose),{1,2});
}
